...
void UpdateManager::handle_activeFades(StationManager& manager) {
    if (manager.m_active_fades.empty()) return;
    auto now = std::chrono::steady_clock::now();
    bool changed = false;

    manager.m_active_fades.erase(std::remove_if(manager.m_active_fades.begin(), manager.m_active_fades.end(),
    ...
    if (changed) manager.m_needs_redraw = true;
}
