...
StateSnapshot StationManager::createSnapshot() const {
    ...
    for (const auto& station : m_stations) {
        snapshot.stations.push_back({ .name = station.getName(), .current_title = station.getCurrentTitle(), .bitrate = station.getBitrate(), .current_volume = station.getCurrentVolume(), .is_initialized = station.isInitialized(), .is_favorite = station.isFavorite(), .is_buffering = station.isBuffering(), .playback_state = station.getPlaybackState() });
    }
    ...
    if (!snapshot.stations.empty()) {
        ...
        if(m_song_history->contains(active_station_name)) snapshot.active_station_history = (*m_song_history)[active_station_name];
        else snapshot.active_station_history = nlohmann::json::array();
    }
    snapshot.auto_hop_total_duration = 0;
    if (!m_stations.empty()) snapshot.auto_hop_total_duration = AUTO_HOP_TOTAL_TIME_SECONDS / static_cast<int>(m_stations.size());
...
void StationManager::actorLoop() {
    ...
    for (int station_idx : m_active_station_indices) if(station_idx >=0 && station_idx < (int)m_stations.size()) m_stations[station_idx].shutdown();
...
void StationManager::handle_navigate(NavDirection direction) {
    ...
    if (m_active_panel == ActivePanel::STATIONS) {
        ...
        if (new_idx != old_idx) {
            ...
            if (current_station.isInitialized() && current_station.getPlaybackState() != PlaybackState::Muted) fadeAudio(old_idx, 0.0, FADE_TIME_MS);
            m_session_switches++;
            m_last_switch_time = std::chrono::steady_clock::now();
        }
        m_active_station_idx = new_idx;
        m_nav_history.push_back({direction, std::chrono::steady_clock::now()});
        if (m_nav_history.size() > MAX_NAV_HISTORY) m_nav_history.pop_front();
        updateActiveWindow();
        m_history_scroll_offset = 0;
    } else {
        size_t history_size = 0;
        if (!m_stations.empty()) { const auto& name = m_stations[m_active_station_idx].getName(); if (m_song_history->contains(name)) history_size = (*m_song_history)[name].size(); }
        if (direction == NavDirection::UP) { if (m_history_scroll_offset > 0) m_history_scroll_offset--; } 
        else { if (history_size > 0 && m_history_scroll_offset < (int)history_size - 1) m_history_scroll_offset++; }
    }
    m_needs_redraw = true;
}
...
void StationManager::handle_toggleMute() {
    ...
    if (station.getPlaybackState() == PlaybackState::Muted) {
        ...
        fadeAudio(m_active_station_idx, station.getPreMuteVolume(), FADE_TIME_MS / 2);
    } else {
        station.setPreMuteVolume(station.getCurrentVolume());
        station.setPlaybackState(PlaybackState::Muted);
        station.setMuteStartTime();
        fadeAudio(m_active_station_idx, 0.0, FADE_TIME_MS / 2);
    }
    m_needs_redraw = true;
}
...
void StationManager::handle_toggleAutoHop() {
    ...
    if (m_auto_hop_mode_active) {
        ...
        if(!m_stations.empty()) {
            ...
            if (station.getPlaybackState() != PlaybackState::Playing) handle_toggleMute();
            if (station.getCurrentVolume() < 50.0) fadeAudio(m_active_station_idx, 100.0, FADE_TIME_MS);
        }
    }
    m_needs_redraw = true;
}
...
void StationManager::handle_toggleFavorite() {
    if (m_active_station_idx >= 0 && m_active_station_idx < (int)m_stations.size()) m_stations[m_active_station_idx].toggleFavorite();
    m_needs_redraw = true;
}
...
void StationManager::handle_toggleDucking() {
    ...
    if (station.getPlaybackState() == PlaybackState::Ducked) {
        station.setPlaybackState(PlaybackState::Playing);
        fadeAudio(m_active_station_idx, station.getPreMuteVolume(), FADE_TIME_MS);
    } else {
        station.setPreMuteVolume(station.getCurrentVolume());
        station.setPlaybackState(PlaybackState::Ducked);
        fadeAudio(m_active_station_idx, DUCK_VOLUME, FADE_TIME_MS);
    }
    m_needs_redraw = true;
}
...
void StationManager::handle_toggleCopyMode() {
    ...
    if (m_copy_mode_active) m_copy_mode_start_time = std::chrono::steady_clock::now();
    m_needs_redraw = true;
}
...
void StationManager::handle_updateAndPoll() {
    ...
    if (m_copy_mode_active) {
        if (std::chrono::duration_cast<std::chrono::seconds>(now - m_copy_mode_start_time).count() >= COPY_MODE_TIMEOUT_SECONDS) handle_toggleCopyMode();
    }
    if (m_auto_hop_mode_active) {
...
void StationManager::handle_activeFades() {
    ...
    for (int i = m_active_fades.size() - 1; i >= 0; --i) {
        ...
        RadioStream* station = (fade.station_id >= 0 && fade.station_id < (int)m_stations.size()) ? &m_stations[fade.station_id] : nullptr;
...
void StationManager::updateActiveWindow() {
    ...
    for (int idx : m_active_station_indices) if (new_active_set.find(idx) == new_active_set.end()) to_shutdown.push_back(idx);
    for (int idx : to_shutdown) shutdownStation(idx);
    for (int idx : new_active_set) if (m_active_station_indices.find(idx) == m_active_station_indices.end()) initializeStation(idx);
    if(m_active_station_idx >= 0 && m_active_station_idx < (int)m_stations.size()) {
        ...
        if (new_station.isInitialized() && new_station.getPlaybackState() != PlaybackState::Muted && new_station.getCurrentVolume() < 99.0) {
            fadeAudio(m_active_station_idx, 100.0, FADE_TIME_MS);
        }
    }
}
void StationManager::initializeStation(int station_idx) { if (station_idx < 0 || station_idx >= (int)m_stations.size()) return; double vol = (station_idx == m_active_station_idx) ? 100.0 : 0.0; m_stations[station_idx].initialize(vol); m_active_station_indices.insert(station_idx); }
void StationManager::shutdownStation(int station_idx) { if (station_idx < 0 || station_idx >= (int)m_stations.size()) return; m_stations[station_idx].shutdown(); m_active_station_indices.erase(station_idx); }
void StationManager::fadeAudio(int station_id, double to_vol, int duration_ms) {
    ...
    m_active_fades.erase(std::remove_if(m_active_fades.begin(), m_active_fades.end(), [station_id](const ActiveFade& f) { return f.station_id == station_id; }), m_active_fades.end());
    station.setFading(true);
    m_active_fades.push_back({ .station_id = station_id, .generation = station.getGeneration(), .start_vol = station.getCurrentVolume(), .target_vol = to_vol, .start_time = std::chrono::steady_clock::now(), .duration_ms = duration_ms });
}
void StationManager::saveHistoryToDisk() { PersistenceManager persistence; persistence.saveHistory(*m_song_history); m_unsaved_history_count = 0; }
void StationManager::addHistoryEntry(const std::string& station_name, const nlohmann::json& entry) { (*m_song_history)[station_name].push_back(entry); m_new_songs_found++; if (++m_unsaved_history_count >= HISTORY_WRITE_THRESHOLD) saveHistoryToDisk(); }
