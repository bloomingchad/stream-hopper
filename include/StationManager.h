...
private:
    ...
    mutable std::mutex m_stations_mutex; std::vector<RadioStream> m_stations;
    std::vector<ActiveFade> m_active_fades; std::unordered_set<int> m_active_station_indices;
    Strategy::Preloader m_preloader; std::unique_ptr<MpvEventHandler> m_event_handler;
    std::unique_ptr<nlohmann::json> m_song_history; int m_unsaved_history_count;
    int m_active_station_idx; bool m_copy_mode_active; ActivePanel m_active_panel;
    int m_history_scroll_offset; std::chrono::steady_clock::time_point m_copy_mode_start_time;
    bool m_auto_hop_mode_active; std::chrono::steady_clock::time_point m_auto_hop_start_time;
    HopperMode m_hopper_mode; std::chrono::steady_clock::time_point m_last_switch_time;
    std::deque<NavEvent> m_nav_history; std::chrono::steady_clock::time_point m_session_start_time;
    int m_session_switches; int m_new_songs_found; int m_songs_copied;
    bool m_was_quit_by_mute_timeout; std::atomic<bool> m_quit_flag;
    std::atomic<bool> m_needs_redraw; std::thread m_actor_thread;
    std::deque<StationManagerMessage> m_message_queue; std::mutex m_queue_mutex;
